...
?>
	...
	<article class="tdf-single">
		...
		<div class="tdf-container tdf-container--narrow">
			 ...
			 * Relationship logic: Gets all tag IDs on the current post, then finds
			 * other posts across all CPTs that share at least one of those tags,
			 * excluding the current post itself.
			 * Fallback: If the post has no tags, or no other posts share its tags,
			 * the section is hidden entirely.
			 */
			$current_tags = wp_get_post_tags(get_the_ID(), ['fields' => 'ids']); /* gets the current post ID and all the tag IDs associated with it not the objects */

			if (! empty($current_tags)) :/* checks to make sure there are tags if there arent  everything is skiped */
				$related_query = new WP_Query([ /* Starts a brand new custom database query, separate from the main page query */
					'post_type'      => ['article', 'opinion', 'review', 'post'],/* Search across all your content types */
					'tag__in'        => $current_tags,/* returns only posts that have at least one tag matching the current post's tags. this is to ensure the suggestions are related to the post and not random unrelated */
					'post__not_in'   => [get_the_ID()],/* Excludes the current post from results so it doesn't recommend the currently viewing article */
					'posts_per_page' => 3,/* Returns only a maximum of 3 other posts with relatated tags */
					'orderby'        => 'relevance',/* Posts sharing more tags with the current post appear first */
				]);
				/* even if the post has tags, this checks that at least one other post actually shares them. If nothing matches, the section is hidden. */
				if ($related_query->have_posts()) : ?>
					<div class="tdf-single__also-like">
						<span class="tdf-single__also-like-label">You may also like</span> <!--  the output on the top of related posts list -->
						<div class="tdf-single__also-like-list">
							<?php while ($related_query->have_posts()) : $related_query->the_post(); ?> <!--  loads the post data depending on how many met the criteria -->
								<a href="<?php the_permalink(); ?>" class="tdf-single__also-like-item">
									<?php if (has_post_thumbnail()) : ?> <!-- checks if the post being suggested has a featured image, if it does it outputs it if not it just outputs the title and date -->
										<div class="tdf-single__also-like-img">
											<?php the_post_thumbnail('medium'); ?> <!-- outputs the featured image of the post being suggested -->
										</div>
									<?php endif; ?>
									<div class="tdf-single__also-like-body">
										<h4 class="tdf-single__also-like-title"><?php the_title(); ?></h4> <!--  outputs the title of the post being suggested	 -->
										<time class="tdf-single__also-like-date"><?php echo get_the_date(); ?></time> <!-- outputs the published date of the post being suggested -->
									</div>
								</a>
							<?php endwhile;
							wp_reset_postdata(); ?> <!-- Resets WordPress back to the main post after the custom query finishes -->
						</div>
					</div>
			<?php endif;
			endif; ?>
